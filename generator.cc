...
#define GENERATOR(M, POLY, TYPE) GaloisFieldTablesGenerator<M, POLY, TYPE>::generate(#TYPE);

template <int M, int POLY, typename TYPE>
struct GaloisFieldTablesGenerator
{
	...
	static void generate(const char *type)
	{
		...
		std::cout << "struct GaloisFieldTables<" << M << ", " << POLY << ", " << type << ">\n{\n";
...
int main()
{
	...
	std::cout << "struct GaloisFieldTables {};" << std::endl << std::endl;
